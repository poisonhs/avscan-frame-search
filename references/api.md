...
基础域名：`https://avscan.cc`（Cloudflare 托管，无需 Cookie/鉴权）。
...
- 最多返回 20 条（服务端上限，无分页；实际可能少于 20，取决于匹配数量。实测 `limit`/`top_k`/`page`/`offset` 等参数均被忽略），按 `best_similarity` 降序。
- `frames` 为该片命中的预览帧，`similarity` 为单帧相似度。
...
def batch_search(images: list[str], out_csv: str):
    ...
    for p in images:
        try:
            r = requests.post("https://avscan.cc/search",
